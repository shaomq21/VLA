...
class BDDLBaseDomain(SingleArmEnv):
    ...
    def _safe_append_sampler(self, comp, sampler, meta=None):
        """
        保险追加：若这个对象已经被该 composite sampler 管过，就跳过。
        同时兜底捕获 robosuite 内部的重复断言（AssertionError）。
        """
        # 尝试通过对象引用做简单去重（不同版本可能没有这些属性，失败就忽略）
        try:
            obj = getattr(sampler, "mujoco_objects", None)
            existing = getattr(comp, "mujoco_objects", None)
            if obj is not None and existing is not None:
                objs = obj if isinstance(obj, (list, tuple, set)) else [obj]
                exist_list = existing if isinstance(existing, (list, tuple, set)) else [existing]
                if any(o in exist_list for o in objs):
                    return
        except Exception:
            pass

        # 兜底：即使没检测到重复，也防止 robosuite 的 AssertionError
        try:
            if meta is None:
                comp.append_sampler(sampler)
            else:
                comp.append_sampler(sampler, meta)
            ...
            if state[0] == "on" and state[2] in regions:
                ...
                if self.is_fixture(object_name):
                    ...
                    if object_name not in self.objects_dict:
                        print("[BDDL WARN] object not found:", object_name,
                              "| available:", list(self.objects_dict.keys()))
                        continue
