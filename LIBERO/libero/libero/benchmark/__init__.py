...
class Benchmark(abc.ABC):
    ...
    def _make_benchmark(self):
        ...
        if self.name == "libero_90":
            ...
            # Filter task_order to only include valid indices (in case we have fewer tasks)
            task_order = task_orders[self.task_order_index]
            valid_order = [i for i in task_order if i < len(tasks)]
            if valid_order:
                print(f"[info] using task orders {valid_order}")
                self.tasks = [tasks[i] for i in valid_order]
            else:
                # If no valid indices (shouldn't happen, but safe fallback), use all tasks
                print(f"[info] using all {len(tasks)} tasks in order")
                self.tasks = tasks
